(function (factory) {
  ...
  const assign_points = (pnts, clstr) => {
    return pnts.map((pnt) => [pnt[0], pnt[1], closest(pnt, clstr)[1]]);
  };

  const update_cluster = (pnts, clstr) => {
    return clstr.map((_, i) => [...mean(pnts.filter((p) => p[2] === i)), i]);
  };

  const initialize_cluster = (pnts, k) => {
    return shuffle(pnts).slice(0, k);
  };
  ...
  let sketch = function (p) {
    ...
    p.draw = function () {
      ...
      const [res_pnts, res_clstr, cost] = run_kmean(pnts, k, 50);
